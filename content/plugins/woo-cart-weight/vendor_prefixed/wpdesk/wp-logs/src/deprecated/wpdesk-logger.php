...
if (!\class_exists('WCWeightVendor\\WPDesk_Logger')) {
    /**
    ...
    class WPDesk_Logger
    {
        ...
        {
            if (!self::$logger) {
                $loggerFactroy = new \WCWeightVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        {
            if (\defined('WCWeightVendor\\WC_VERSION')) {
                if (\version_compare(\WCWeightVendor\WC_VERSION, '3.0', '<')) {
