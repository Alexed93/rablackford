...
namespace WCWeightVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use WCWeightVendor\WPDesk\Helper\Debug\LibraryDebug;
use WCWeightVendor\WPDesk\Helper\Integration\LicenseIntegration;
use WCWeightVendor\WPDesk\Helper\Integration\LogsIntegration;
use WCWeightVendor\WPDesk\Helper\Integration\SettingsIntegration;
use WCWeightVendor\WPDesk\Helper\Integration\TrackerIntegration;
use WCWeightVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use WCWeightVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \WCWeightVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    const LIBRARY_TEXT_DOMAIN = 'woo-cart-weight';
    ...
    {
        $helper_info = new \WCWeightVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \WCWeightVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \WCWeightVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \WCWeightVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \WCWeightVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \WCWeightVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        $library_debug_info = new \WCWeightVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \WCWeightVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \WCWeightVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
