...
if (empty($notice_content)) {
    ?>
            <?php 
    \printf(\__('Hey %s,', 'woo-cart-weight'), $username);
    ?><br/>
            <?php 
    \_e('We need your help to improve <strong>WP Desk plugins</strong>, so they are more useful for you and the rest of <strong>30,000+ users</strong>. By collecting data on how you use our plugins, you will help us a lot. We will not collect any sensitive data, so you can feel safe.', 'woo-cart-weight');
    ?>
            <a href="<?php 
    echo $terms_url;
    ?>" target="_blank"><?php 
    \_e('Find out more &raquo;', 'woo-cart-weight');
    ?></a>
        <?php 
} else {
    ?>
